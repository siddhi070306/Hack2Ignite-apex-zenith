...
- **Condition-specific education content**: when advice was RAG-grounded, a "Learn More" section shows the fuller corpus guidance for that topic, translated into the patient's selected language.
- **Follow-up worklist**: an optional follow-up date (auto-suggested by urgency) can be set when saving a triage record; the ASHA Dashboard surfaces a "Follow-ups Due" list for anything past that date and not yet marked done. In-app only — no SMS/push notifications.
- **Doctor pre-consultation handoff packet**: opening any record — from the doctor's verification flow or the shared detail view either role can open — shows that patient's prior visit history, so a doctor isn't starting the case cold.
